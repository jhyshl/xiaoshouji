...
function formatPhoneMessages(messages) {
  return (Array.isArray(messages) ? messages : [])
    ...
    .slice(-120)
    .map((message) => {
      const role = message.role === "assistant" ? "角色" : "玩家";
  ...
  try {
    ...
    if (
      ...
      try {
        ...
        runtime.lastError = `历史回填未完成：${error.message || "总结失败"}`;
...
function render() {
  ...
  if (runtime.memoryStatus) {
    ...
    runtime.memoryStatus.textContent = identity
      ? `跨端记忆：酒馆总结${
          memory?.summary && !memory?.summaryStale ? "已注入" : "暂无"
        } · 小手机总结${phoneSummaryReady ? "已注入" : "暂无"} · 小手机实时 ${phoneLiveCount} 条`
      : "跨端记忆：等待打开角色聊天";
  }
